...
public class InteractableWater : MonoBehaviour
{
    ...
    private const int NUM_OF_V_VERTICES = 2;
    ...
    public void GenerateMesh()
    {
        ...
        //construct triangles
        int[] triangles = new int[(NumOfVertices - 1) * (NUM_OF_V_VERTICES - 1) * 6];
        int index = 0;

        for(int y = 0; y < NUM_OF_V_VERTICES - 1; y++)
        {
...
public class InteractableWaterEditor : Editor
{
    ...
    public override VisualElement CreateInspectorGUI()
    {
        ...
        // Add a button to regenerate the mesh
        root.Add(new VisualElement { style = { height = 10 } });

        Button regenerateMeshButton = new Button(() => _water.GenerateMesh())
        {
            text = "Regenerate Mesh"
        };
        root.Add(regenerateMeshButton);
    ...
    private void ChangeDimensions(ref float with, ref float height, float calculatedWithMax, float calculatedHeightMax)
    {
        with = Mathf.Max(0.1f, calculatedWithMax);
    ...
    private void OnSceneGUI()
    {
        ...
        EditorGUI.BeginChangeCheck();
        Vector3 newBottomRight = Handles.FreeMoveHandle(corners[01], handleSize, snap, Handles.CubeHandleCap);
        if (EditorGUI.EndChangeCheck())
        {
        ...
        //Update the mesh if dimensions are moved
        if (GUI.changed)
        {
            _water.GenerateMesh();
        }
